...
class HexMujocoArcherL6Y(HexMujocoBase):
    ...
    def __init__(
        ...
        # state init - L6Y has 12 robot joints: 6 arm + 6 gripper (with equality constraints)
        # We track 7 actuated joints: joint_1-6 (arm, idx 0-5) + gripper_left_joint_1 (idx 8)
        self.__state_robot_idx = [0, 1, 2, 3, 4, 5, 8]  # 7 actuated joints
        self.__state_obj_idx = [12, 13, 14, 15, 16, 17, 18]  # free body (qpos idx 12-18)
        self.__ctrl_robot_idx = [0, 1, 2, 3, 4, 5, 6]  # 7 actuators (6 arm + 1 gripper)
