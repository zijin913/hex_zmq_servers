...
# Based on HexMujocoArcherL6Y (single-arm) and HexMujocoE3Desktop (dual-arm pattern).
################################################################
...
class HexMujocoArcherL6YDual(HexMujocoBase):
    ...
    def __init__(
        ...
        # State indices — L6Y dual: 12 qpos per arm (6 arm + 6 gripper with equality)
        # Left arm: joint_1-6 at qpos 0-5, gripper_left_joint_1 at qpos 8
        # Right arm: joint_1-6 at qpos 12-17, gripper_left_joint_1 at qpos 20
    ...
    def close(self):
        ...
        if not self.__headless:
            self.__viewer.close()
        hex_log(HEX_LOG_LEVEL["info"], "HexMujocoArcherL6YDual closed")
